...
## 0 基础背景
**前端**: 通过**词法分析和语法分析**, 将源代码解析成抽象语法树 (abstract syntax tree, **AST**). 通过**语义分析**, 扫描抽象语法树, 检查其是否存在语义错误.
...
**语法分析的目的**, 按照**程序的语法规则**,  （例如, 对于 SysY 程序, 关键字 int 后跟的一定是一个标识符, 而不可能是一个整数字面量）,**将输入的 token 流变成程序的 AST**
